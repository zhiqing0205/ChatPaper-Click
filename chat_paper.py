...
class Reader:
    ...
    def __init__(self, key_word, query, filter_keys,
                 root_path='./',
                 gitee_key='',
                 sort=arxiv.SortCriterion.SubmittedDate, user_name='defualt', language='zh', file_format='md', save_image=False):
...
def chat_paper_function(pdf_path, query='all: ChatGPT robot', key_word='machine learning', filter_keys='ChatGPT robot', max_results=1, sort='Relevance', save_image=False, file_format='md', language='zh'):
    # 创建一个Reader对象，并调用show_info方法
    if sort == 'Relevance':
    ...
    reader1 = Reader(key_word=key_word, query=query, filter_keys=filter_keys, sort=sort_criterion)
    ...
    if pdf_path:
        ...
        filter_results = reader1.filter_arxiv(max_results=max_results)
        paper_list = reader1.download_pdf(filter_results)
        reader1.summary_with_chat(paper_list=paper_list)

    return paper_list  # 或者返回其他适合的结果

# chat_paper_function(pdf_path='./pdf_files/2023/12/19/a5fe09ca59d7a14d5b61eefc85ade7bf.pdf')